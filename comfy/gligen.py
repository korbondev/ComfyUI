...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4, glu=False, dropout=0.0):
        ...
        project_in = nn.Sequential(ops.Linear(dim, inner_dim), nn.GELU()) if not glu else GEGLU(dim, inner_dim)

        self.net = nn.Sequential(project_in, nn.Dropout(dropout), ops.Linear(inner_dim, dim_out))
...
class Gligen(nn.Module):
    ...
    def set_position(self, latent_image_shape, position_params, device):
        ...
        return self._set_position(box_out.to(device), masks.to(device), conds.to(device))
...
def load_gligen(sd):
    ...
    for a in ["input_blocks", "middle_block", "output_blocks"]:
        for b in range(20):
            k_temp = filter(lambda k: "{}.{}.".format(a, b) in k and ".fuser." in k, sd_k)
