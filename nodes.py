    ...
    def save_images(
        ...
        if "COMFY_STATIC_IMAGE_FILE" in os.environ and dumped_prompt is not None:
            static_filename = os.path.join(
                full_output_folder, os.environ["COMFY_STATIC_IMAGE_FILE"]
            )
            # add the prompt_id to the filename
            print(dumped_prompt)
            static_filename = static_filename.replace(
                ".png", f"_{dumped_prompt.prompt_id}.png"
            )
        ...
        for batch_number, image in enumerate(images):
            ...
            if metadata is not None:
                ...
                success, img = numpy_array_to_fpng(
                    data, filename=os.path.join(full_output_folder, file)
                )
                if not success:
